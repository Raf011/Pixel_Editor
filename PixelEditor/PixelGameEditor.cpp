...
bool PixelGameEditor::OnEditorTick(float & fElapsedTime)
{
	...
	if(!GameWorld->isSimulating())
	{
		...
		{
			GameWorld->DestroyGameObject(SelectedGameObject);
	...
	else
	{
		...
		if(GetKey(olc::Key::M).bHeld || playPhysics)
		{
...
void PixelGameEditor::FocusAndCenterCameraOnEntity(GameObject* Entity)
{
...
{
	for (auto i : GameWorld->GetGameObjectsSpawnedInTheWorld())
	{
		i->Tick(fElapsedTime);
	}
}
...
{
	if (GetMouse(0).bHeld && CanDrag && !GameWorld->isMouseDragDisabled())
	{
		if (!isObjectDraggedByMouse)
		{
			for (auto obj : GameWorld->GetGameObjectsSpawnedInTheWorld())
			{
...
void PixelGameEditor::UpdateDebugText()
{
	...
	//...........World Window - aka 'Game Objects' window...........//
	WorldWindow->Clear();
	...
	//engine->DrawString(0, 500, "World ID: " + std::to_string(GameWorld->debug_retunNextAvailableID()), olc::RED, 2);
...
void PixelGameEditor::UpdatePhysics(float &timeElapsed)
{
	float singleStep = 0.016f + 0.016f * timeElapsed;
	World::Get()->GetPhysicsWorld()->Step(singleStep, 1, 6);
}
