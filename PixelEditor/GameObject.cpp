...
void GameObject::Tick(float &elapsedTime)
{
	...
	{
		if (SimulatesPhysics)//objet simluates physics
		{
			if(!overridePhysicsPos)//use the physics engine to override the position of the game object
			{
				UsePhysicsEngineToUpdateThePosition();
			}

			else //don't update the position of the object
			{
				physicsBody->SetTransform({ -GetPosition().X, -GetPosition().Y }, GetRotation());
			}
		}
		...
		if (mainCam && !despawned && Visible)
		{
			...
			if (isAnimated)
			{
				...
				engine->DrawPartialRotatedDecal({ transform.PositionOffset.X, transform.PositionOffset.Y }, decal, pos, size, scale, transform.fRotation + SpriteRotationOffset, { size.x / 2, size.y / 2 }, pTint);
			...
			{
				engine->DrawRotatedDecal({ transform.PositionOffset.X, transform.PositionOffset.Y }, decal, transform.fRotation + SpriteRotationOffset, { transform.Size.X / 2, transform.Size.Y / 2 }, { transform.OffsetSize.X, transform.OffsetSize.Y }, pTint);
...
void GameObject::UsePhysicsEngineToUpdateThePosition()
{
	transform.Position = { -(physicsBody->GetPosition().x), -(physicsBody->GetPosition().y) };
	transform.fRotation = physicsBody->GetAngle();
}
...
void GameObject::ChangeSprite(std::string newSpritePath, float spriteRotation, bool animate, float animSpeed_ms)
{
	delete sprite;
	sprite = new olc::Sprite(newSpritePath);

	delete decal;
	decal = new olc::Decal(sprite);

	SpriteRotationOffset = spriteRotation;
...
void GameObject::SetSpriteRotation(float newRotationOffset)
{
	SpriteRotationOffset = newRotationOffset;
}

void GameObject::SetSpriteSizeToObjectSize()
{
	sprite->width  = transform.Size.X;
	sprite->height = transform.Size.Y;
}

void GameObject::AddPhysicsComponent(float Density, float Friction)
{
	//GameWorld->GetPhysicsWorld()->CreateBody
	if(!SimulatesPhysics)
	{
		b2BodyDef bodyDef;
		bodyDef.type = b2_dynamicBody;
		bodyDef.position.Set(-(transform.Position.X), -(transform.Position.Y)); //need to negate float as PixelEngine uses oposite coord system

		b2PolygonShape dynamicShape;
		dynamicShape.SetAsBox(CollsionSize.X / 2.0f, CollsionSize.Y / 2.0f); //b2d takes only half of the dimentions
...
void GameObject::Physics_ChangeDensity(float NewDensity)
{
	if(SimulatesPhysics)
	{
		physicsFixture->SetDensity(NewDensity);
		physicsBody->ResetMassData();
	}
}

void GameObject::OverridePhysicsBodyPosition(bool override)
{
	if(override)
	{
		physicsBody->SetGravityScale(0.0f);
		overridePhysicsPos = true;
	}
	else
	{
		physicsBody->SetGravityScale(1.0f);
		overridePhysicsPos = false;
	}
}
